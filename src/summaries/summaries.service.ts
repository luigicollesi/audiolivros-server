...
export class SummariesService {
  ...
  async findByTitleAndLanguage(title: string, language: 'pt-BR' | 'en-US'): Promise<SummaryItem[]> {
    ...
    if (ids.length === 0) throw new NotFoundException(`Nenhum book_id para "${title}" em ${language}.`);

    const bookIds = Array.from(new Set(ids.map(r => r.book_id).filter(Boolean)));
    ...
    if (items.length === 0) throw new NotFoundException(`Nenhum summary para "${title}" em ${language}.`);
    // opcional dedupe
